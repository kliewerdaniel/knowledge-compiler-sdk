...
def generate(build_dir: str, app: dict, emit: DiagnosticEmitter) -> None:
    ...
    # ---- root landing page (always present; links to generated sections) --
    # Guarantees a valid "/" route even if the model declares no root page.
    home_rel = os.path.join(app_root, "app", "page.tsx")
    if not os.path.exists(home_rel):
        route_links = "\n".join(
            f'      <li><a href="{p.get("route", "/")}">{p.get("title", p.get("id"))}</a></li>'
            for p in (app.get("pages", []) or [])
        ) or '      <li><a href="/evaluation">Evaluation</a></li>'
        home = (
            ...
            "        Generated by the Knowledge Compiler from your source corpus.\n"
            "      </p>\n"
            "      <h2>Sections</h2>\n"
            "      <ul>\n"
            f"{route_links}\n"
        ...
        _write(app_root, "app/page.tsx", home)
